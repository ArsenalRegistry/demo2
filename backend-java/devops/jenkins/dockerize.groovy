...
podTemplate(label: label, serviceAccount: 'default', namespace: 'appdu-demo3',
    ...
    volumes: [
        ...
        // nfsVolume(mountPath: '/home/jenkins', serverAddress: '10.217.166.78', serverPath: '/jenkins_slave', readOnly: false)
        ]
    ) {
    node(label) {
        ...
        try {
            ...
            // Image - Docker 이미지 명칭
            def imageName           = "image-registry.openshift-image-registry.svc:5000/appdu-demo3/backend-java"
            // Maven Setting
            ...
            stage('Get Source') {
                ...
                """
                git url: "http://172.30.216.150/appdu-demo3/backend-java.git",
            ...
            stage('Tagging Version') {
                ...
                try {
                    ...
                    withCredentials([
                        ...
                        sh """
                            sed -i "s/^version.*/version: ${tagName}/g" devops/helm/backend-java/values.yaml
